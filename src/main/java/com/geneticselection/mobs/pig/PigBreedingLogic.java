...
public class PigBreedingLogic {
    public static PigEntity breed(PigEntity parent1, PigEntity parent2, ServerWorld world) {
        ...
        PigEntity offspring = (PigEntity) EntityType.Pig.create(world);
        ...
        if (offspring != null) {
            ...
            LOGGER.info("Breeding Pig: Parent1 ID={}, Parent2 ID={}", parent1.getId(), parent2.getId());
    ...
    private static void applyAttributes(PigEntity Pig, MobAttributes attributes) {
        var speedAttribute = Pig.getAttributeInstance(net.minecraft.entity.attribute.EntityAttributes.GENERIC_MOVEMENT_SPEED);
        if (speedAttribute != null) {
            speedAttribute.setBaseValue(attributes.getMovementSpeed());
        }

        var healthAttribute = Pig.getAttributeInstance(net.minecraft.entity.attribute.EntityAttributes.GENERIC_MAX_HEALTH);
        if (healthAttribute != null) {
            healthAttribute.setBaseValue(attributes.getMaxHealth());
        }

        Pig.setHealth((float) attributes.getMaxHealth());
    }

    private static void influenceGlobalAttributes(PigEntity offspring) {
        if (offspring != null) {
            MobAttributes childAttributes = ((IGeneticEntity) offspring).getMobAttributes();
            if (childAttributes != null) {
                MobAttributes globalAttributes = GlobalAttributesManager.getAttributes(EntityType.PIG);
                ...
                GlobalAttributesManager.updateGlobalAttributes(EntityType.PIG, globalAttributes);
                LOGGER.info("Updated global Pig attributes: Speed={}, Health={}",
