...
public class CustomVindicatorEntity extends VindicatorEntity implements AttributeCarrier {
    ...
    // Base vindicator attack damage (vanilla iron axe ~13 damage)
    private static final double VANILLA_VINDICATOR_ATTACK = 13.0;

    public CustomVindicatorEntity(EntityType<? extends VindicatorEntity> entityType, World world) {
        ...
        this.BaseAttack = VANILLA_VINDICATOR_ATTACK;
        this.generation = 0;

        this.breedingCooldown = 8000 + (int)((1 - (ELvl / 100.0)) * 4000) + random.nextInt(2001);
    ...
    private void applyBonuses() {
        ...
        // Vindicators get stronger generation bonus (0.2 per gen, cap at +10)
        double generationBonus = Math.min(this.generation * 0.2, 10.0);
    ...
    private void applyAttackDamageScaling() {
        ...
        // Vindicators have better energy scaling (70%-100%)
        double energyRatio = (this.MaxEnergy > 0) ? (this.ELvl / this.MaxEnergy) : 1.0;
        double energyMultiplier = 0.7 + (energyRatio * 0.3);
    ...
    private void updateDescription(CustomVindicatorEntity ent) {
    ...
    public ActionResult interactMob(PlayerEntity player, Hand hand) {
        ...
        if (itemStack.isOf(Items.EMERALD) && this.ELvl < this.MaxEnergy) {
            ...
            updateEnergyLevel(this.ELvl + 25.0); // Vindicators get more energy per emerald
            return ActionResult.SUCCESS;
        }
    ...
    public void onSuccessfulKill() {
        if (!this.getWorld().isClient) {
            ...
            // Higher energy gain for vindicators
            double energyGain = 8.0 + Math.random() * 7.0;
            updateEnergyLevel(this.ELvl + energyGain);

            this.heal(this.getMaxHealth() * 0.15f);

            // Higher evolution chance
            float evolutionChance = 0.10f + (float)Math.min(this.killCount, 80) / 1600.0f;
            if (this.random.nextFloat() < evolutionChance) {
                int statChoice = this.random.nextInt(3);
                switch(statChoice) {
                    case 0: bonusAttack += 0.15 + random.nextDouble() * 0.25; break;
                    case 1: bonusHealth += 0.8 + random.nextDouble() * 1.5; break;
                    case 2: bonusSpeed += 0.003 + random.nextDouble() * 0.004; break;
                }

                this.bonusAttack = Math.min(this.bonusAttack, 15.0);
                this.bonusHealth = Math.min(this.bonusHealth, 40.0);
    ...
    public void tick() {
        ...
        if (!this.getWorld().isClient) {
            ...
            if (wasRecentlyHit) {
                updateEnergyLevel(this.ELvl * 0.88); // Lose 12%
                wasRecentlyHit = false;
            }

            // Vindicators drain energy faster (more aggressive)
            if (Math.random() < 0.06) {
                updateEnergyLevel(this.ELvl - 0.10);
            }

            if (ELvl >= MaxEnergy * 0.93 && this.getHealth() < this.getMaxHealth()) {
                this.heal(0.20F);
            }

            // Breeding
            if (ELvl >= 85.0 && ticksSinceLastBreeding >= breedingCooldown) {
                double searchRadius = 48.0;

                List<CustomVindicatorEntity> mateCandidates = this.getWorld().getEntitiesByClass(
                        CustomVindicatorEntity.class,
                        this.getBoundingBox().expand(searchRadius),
                        candidate -> candidate != this &&
                                candidate.getEnergyLevel() >= 85.0 &&
                                candidate.ticksSinceLastBreeding >= candidate.breedingCooldown
                );

                if (!mateCandidates.isEmpty()) {
                    CustomVindicatorEntity nearestMate = null;
                    double minDistanceSquared = Double.MAX_VALUE;

                    for (CustomVindicatorEntity candidate : mateCandidates) {
                    ...
                    if (nearestMate != null && minDistanceSquared < 16.0) {
                        if (this.getWorld() instanceof ServerWorld serverWorld) {
                            CustomVindicatorEntity child = createChild(serverWorld, nearestMate);
                            if (child != null) {
            ...
            if (ELvl <= 0.0) {
                this.damage(this.getDamageSources().starve(), 2.0f);
            }
            ...
            double newSpeed = Math.max(calculatedSpeed, this.Speed * 0.5);
    ...
    @Nullable
    public CustomVindicatorEntity createChild(ServerWorld serverWorld, CustomVindicatorEntity mate) {
        CustomVindicatorEntity parent1 = this;
        CustomVindicatorEntity parent2 = mate;
        CustomVindicatorEntity child = ModEntities.CUSTOM_VINDICATOR.create(serverWorld);
        ...
        double inheritanceFactor = Math.max(0.5, Math.min(parent1.getEnergyLevel(), parent2.getEnergyLevel()) / 100.0);
        double randomFactor = 0.92 + random.nextDouble() * 0.16;
        ...
        child.bonusAttack = Math.max(0, avgBonusAttack * inheritanceFactor * randomFactor * 1.10); // 10% breeding bonus
        ...
        child.killCount = (int)(avgKillCount * inheritanceFactor * 0.6);
        ...
        child.breedingCooldown = Math.max(4000, childBreedingCooldown);
        child.ticksSinceLastBreeding = 0;

        child.applyBonuses();

        parent1.updateEnergyLevel(parent1.getEnergyLevel() * 0.55);
        parent2.updateEnergyLevel(parent2.getEnergyLevel() * 0.55);
    ...
    public void readCustomDataFromNbt(NbtCompound nbt) {
        ...
        this.BaseAttack = nbt.contains("BaseAttack") ? nbt.getDouble("BaseAttack") : VANILLA_VINDICATOR_ATTACK;
